...
~~~javascript
let sqlStr = `select id,password from member where username='${params.username}'`;
db.query(sqlStr, (err, rst)=>{
});
~~~
...
let jwtStr = jwt.sign({ uid: rst[0].id }, CONFIG.JWT_SECRET, { expiresIn: CONFIG.JWT_EXPIR });
~~~
...
~~~javascript
//将jwt字符串写入返回值
CONFIG.USER_LOGIN.jwt = jwtStr;

//返回结果
return res.send(CONFIG.USER_LOGIN);
~~~
